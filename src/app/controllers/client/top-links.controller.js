...
export const getClientTopLinks = async (req, res) => {
    try {
        const { period, timeline = 'current' } = req.query // period: 'week' or 'month', timeline: 'past', 'current'
        const userId = req.currentUser._id // Lấy user_id từ currentUser được set bởi middleware
        const now = new Date()
        ...
        console.log('Date range:', { startDate, endDate, period, timeline, userId }) // Debug log

        // Aggregate để tính toán thống kê cho client
        const clicklogsCollection = mongoose.connection.db.collection('clicklogs')
        const topLinks = await clicklogsCollection.aggregate([
            {
                $match: {
            ...
            },
            {
                $unwind: {
                    path: '$link_info',
                    preserveNullAndEmptyArrays: false // Chỉ lấy các click logs có link_info
                }
            },
            {
                $group: {
        ...
        console.log('Client aggregation result:', topLinks) // Debug log
        ...
        console.error('Error getting client top links:', error)
        res.status(500).json({
            status: 500,
            success: false,
            message: 'Error getting client top links',
            error: error.message,
            data: null
        })
    }
